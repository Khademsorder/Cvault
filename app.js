...
// =================================================================
// EMERGENCY FIXES - PASTE AT THE VERY END OF app.js
// =================================================================

// FIX 1: Replace the entire setupEventListeners function
(function() {
    // Save the original function if it exists
    const originalSetupEventListeners = window.setupEventListeners;
    
    // Define new fixed version
    window.setupEventListeners = function() {
        console.log('🎯 [FIXED] Setting up event listeners...');
        ...
        window.addEventListener('click', handleGlobalClick);
        window.addEventListener('blur', () => {
            if (window.appState) window.appState.lastActivity = Date.now();
        });
        
        // Get elements dynamically instead of relying on cached elements
        const elements = {
            ...
            storageInfoBtn: document.getElementById('storageInfoBtn'),
        ...
        document.querySelectorAll('.pin-key').forEach(key => {
            key.addEventListener('click', function(e) {
                ...
                if (pinInput) {
                    const currentPin = pinInput.textContent || '';
                    if (key === 'clear') {
                        ...
                        if (window.handlePinSubmit) {
                            window.handlePinSubmit(currentPin);
                        }
                    } else if (currentPin.length < 4) {
                        pinInput.textContent = currentPin + key;
        ...
        if (elements.googleConnectBtn) {
            elements.googleConnectBtn.addEventListener('click', function() {
                ...
                if (modal) {
                    ...
                    console.error('googleConnectModal not found');
                }
            });
        }
        ...
        if (elements.searchInput) {
            elements.searchInput.addEventListener('input', debounce(function() {
                if (window.handleSearch) {
                    window.handleSearch({target: this});
                }
            }, 300));
        }
        ...
        console.log('✅ [FIXED] Event listeners setup complete');
        ...
        if (originalSetupEventListeners) {
            try {
                ...
                console.warn('Original setupEventListeners failed:', e);
            }
        }
    };
    
    console.log('🔧 Fixed setupEventListeners loaded');
})();

// FIX 2: Define missing modal functions
(function() {
    // Make sure these functions exist globally
    if (typeof window.showGoogleConnectModal === 'undefined') {
        window.showGoogleConnectModal = function() {
            console.log('showGoogleConnectModal called');
            const modal = document.getElementById('googleConnectModal');
            if (modal) {
                ...
                console.error('googleConnectModal element not found');
            }
        };
    }
...
// FIX 3: Override the init function to use our fixes
(function() {
    // Store original init if it exists
    const originalInit = window.VaultOS ? window.VaultOS.init : null;
    
    if (window.VaultOS && window.VaultOS.init) {
        const originalInitFunc = window.VaultOS.init;
        
        window.VaultOS.init = function() {
            console.log('🚀 [PATCHED] Vault OS v5.0 Initializing with fixes...');
            
            try {
                // Call original init
                return originalInitFunc.apply(this, arguments);
            } catch (error) {
                console.error('❌ Initialization failed:', error);
                ...
                const errorHtml = `
                    <div style="text-align: center; padding: 40px; color: white;">
                        ...
                        <h1 style="margin: 20px 0;">INITIALIZATION ERROR</h1>
                        <div style="background: rgba(255,0,0,0.1); padding: 15px; border-radius: 8px; margin: 20px; text-align: left;">
                            <p style="color: #ff6b6b; margin: 5px 0;"><strong>${error.message}</strong></p>
                        </div>
                        <button onclick="location.reload()" 
                                style="padding: 12px 24px; background: #00d4ff; border: none; color: black; border-radius: 5px; cursor: pointer; margin: 5px;">
                            🔄 Reload Page
                        </button>
                        <button onclick="console.log('Error:', ${JSON.stringify(error.toString())}); console.trace();" 
                                style="padding: 12px 24px; background: #333; border: 1px solid #555; color: white; border-radius: 5px; cursor: pointer; margin: 5px;">
                            🐛 Show Error Details
                        </button>
                    </div>
                `;
        ...
        console.log('🔧 VaultOS.init function patched');
    }
})();

// FIX 4: Ensure DOM is ready and basic functions exist
document.addEventListener('DOMContentLoaded', function() {
    console.log('🏠 DOM Ready - Applying emergency fixes');
    ...
    if (!document.getElementById('modalOverlay')) {
        ...
        console.log('✅ Created missing modalOverlay');
    }
    ...
    if (missing.length > 0) {
        ...
        // Try to fix loading screen
        if (!document.getElementById('loadingScreen')) {
            const loadingDiv = document.createElement('div');
            loadingDiv.id = 'loadingScreen';
            loadingDiv.className = 'loading-screen active';
            ...
            console.log('✅ Created missing loadingScreen');
        }
    }
    ...
    window.debugApp = function() {
        ...
        console.log('VaultOS:', window.VaultOS ? 'Available' : 'Not available');
        console.log('CONFIG:', window.CONFIG ? 'Available' : 'Not available');
        console.log('DOM Ready State:', document.readyState);
        
        // Check key elements
        const elementsToCheck = [
            'menuToggle', 'sidebar', 'googleConnectBtn', 
            'googleConnectModal', 'settingsModal', 'uploadModal'
        ];
...
window.addEventListener('error', function(event) {
    ...
    // Don't show alert for known errors
    if (event.message.includes('ConnectModal') || 
        event.message.includes('addEventListener')) {
        event.preventDefault();
        console.log('⚠️ Known error, continuing...');
    }
});

console.log('🛠️ Emergency fixes loaded successfully');
